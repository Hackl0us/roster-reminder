...
def today_roster():
	...
	num_of_today_mute_stash_roster = c.execute("SELECT COUNT(*) FROM stand_up_roster_stash WHERE mute_for_today = 1").fetchone()[0]
	...
	db.close()
	print("today roster: roster_id:" + str(roster['id']) + ", name: " + roster['name'] + ", slack: " + roster['slack_member_id'] + ", type: " + roster['type'] )
	return roster
...
def move_to_next_roster(is_stash = False):
	...
	if (is_stash):
		...
		if (roster['type'] == "NORMAL"):
			print("BRANCH 2")
			c.execute("INSERT INTO stand_up_roster_stash (name, slack_member_id, mute_for_today) VALUES (?, ?, 1)", (roster['name'], roster['slack_member_id']))
			db.commit()
...
def move_flag_to_next_roster(slack_member_id):
	...
	if (slack_member_id == last_roster_id):
		c.execute("UPDATE stand_up_roster SET is_today = 1 WHERE id = (SELECT id FROM stand_up_roster ORDER BY id ASC LIMIT 1)")
		db.commit()
	else:
		c.execute("UPDATE stand_up_roster SET is_today = 1 WHERE id = (SELECT id FROM stand_up_roster WHERE id > ? ORDER BY id LIMIT 1)", (slack_member_id,))
		db.commit()
